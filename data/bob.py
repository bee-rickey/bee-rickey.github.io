...
def compare_with_gospel():
    ...
    print(df.groupby(['Detected State', 'Detected District', 'Current Status','Date Announced'])['Num Cases'].sum().to_csv("test.csv"))

    '''
    dis_counts = pd.pivot_table(df,values = 'Num Cases',
                               index = 'District_Key',
                               columns=['Current Status','Date Announced'],
                               aggfunc = sum).reset_index()

    dis_counts.rename(columns={'Hospitalized':'Confirmed'},inplace=True)
    dis_counts.to_csv("test.csv", index=False)
    '''
    return
